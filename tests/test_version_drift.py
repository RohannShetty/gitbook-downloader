...
Single source of truth for DocHarvest version: ``11.0.3``.
...
- ``docs/lib/version.ts`` and ``src/gitbook_downloader/__init__.py`` are the
  canonical sources and MUST equal ``11.0.3`` (we assert equality, not just
  presence).
- ``frontend/index.html`` <title> must read ``DocHarvest v11.0.3``.

Drift signals (these MUST all read ``11.0.3`` after Phase 1 step 1):
...
# Files where the file MUST contain the literal ``11.0.3`` somewhere.
...
def test_cli_version_fallback_uses_canonical_value() -> None:
    """The direct-script fallback in cli.py MUST equal 11.0.3 (not 9.0.0b1)."""
    ...
    assert "__version__ = \"11.0.3\"" in cli_text, (
        "cli.py direct-script fallback should be 11.0.3, not 9.0.0b1"
    )
