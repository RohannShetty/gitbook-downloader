...
describe('version constants', () => {
  it('should export VERSION as 11.0.1', () => {
    expect(VERSION).toBe('11.0.1');
  });

  it('should export DOWNLOAD_URLS with correct Windows URL', () => {
    expect(DOWNLOAD_URLS.windows).toBe(
      'https://github.com/RohannShetty/gitbook-downloader/releases/download/v11.0.1/docharvest-windows-latest.exe'
    );
  });

  it('should export DOWNLOAD_URLS with correct Linux URL', () => {
    expect(DOWNLOAD_URLS.linux).toBe(
      'https://github.com/RohannShetty/gitbook-downloader/releases/download/v11.0.1/docharvest-ubuntu-latest'
    );
  });

  it('should export DOWNLOAD_URLS with correct macOS URL', () => {
    expect(DOWNLOAD_URLS.macos).toBe(
      'https://github.com/RohannShetty/gitbook-downloader/releases/download/v11.0.1/docharvest-macos-latest'
    );
  });
